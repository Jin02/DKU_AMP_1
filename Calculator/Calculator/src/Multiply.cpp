...
bool Multiply::Execution()
{
...

	{
		...
		char logBuffer[64] = {0, };
		sprintf(logBuffer, "Hi = {R[%d](0x%x) * R[%d](0x%x)}(63:32) = 0x%x; // Lo = {R[%d](0x%x) * R[%d](0x%x)}(31:0) = 0x%x", _rs, rsData, _rt, rtData, (result & 0xffffffff00000000) >> 32, _rs, rsData, _rt, rtData, (result & 0x00000000ffffffff));
		GlobalDumpLogManager->AddLog(logBuffer, true);
		GlobalDumpManagerAddLog3NewLine;
	}
